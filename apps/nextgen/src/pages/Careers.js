    ...
    <div className={styles.careerPage}>
      ...
      <section className={styles.jobsSection} id="jobs">
        <div className={styles.jobsInner}>
          ...
          <div className={styles.jobsList}>
            {filtered.map((job, i) => (
              <JobCard key={job.id} job={job} delay={i * 80} onApply={() => navigate(`/careers/apply/${job.id}`)} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className={styles.ctaSection}>
        <div className={styles.ctaContent}>
          ...
          <button className={styles.btnPrimary} style={{ fontSize: '1.1rem', padding: '16px 44px' }} onClick={() => navigate('/careers/apply/general')}>
            Send Your Resume
          </button>
        </div>
      </section>
